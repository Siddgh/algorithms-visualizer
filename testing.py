...
from algorithms.selection_sort import perform_and_display_selection_sort
from algorithms.insertion_sort import perform_and_display_insertion_sort
from algorithms.merge_sort import perform_and_display_merge_sort
from algorithms.quick_sort import perform_and_display_quick_sort
from algorithms.radix_sort import perform_and_display_radix_sort
from algorithms.heap_sort import perform_and_display_heap_sort
...
input_elements = st.empty()
sorted_elements = st.empty()
...
if len(algorithms_to_run) > 0:
    isAutoGenElement = st.checkbox("Auto Generate Elements?", value=True)

    if isAutoGenElement:
        x = np.linspace(0, 10, 100)
        amount = st.slider("Amount of Elements: ", 5, 30, step=1)
        lst = np.random.randint(0, 100, amount)
    else:
        input_values_raw_string = st.text_input("Enter Elements to sort")
        st.text("Sample input: [3,4,2,1]")
    ...
    time_interval = st.slider("Speed (ms)", min_value=1, max_value=1000, step=1)
    time_interval_sec = time_interval / 1000
    st.text(time_interval_sec)
    should_run = st.button("Run", type="primary")

    st.subheader("Parameters")
    st.write(f"Input Elements:")
    st.text(lst)

    st.text("Sorted Elements:")
    sorted_elements = st.empty()
            ...
            if (i + 1) % 2 != 0:
                with col1:
                    result, time_taken = algorithm_to_run(
                        x, lst, algorithms_to_run[i], time_interval_sec
                    )
